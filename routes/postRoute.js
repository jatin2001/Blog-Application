            ...
            if(err) console.log(err);
                ...
                User.findById(post.UserId,(err,user)=>{
                    if(err) console.log(err);
                        ...
                        req.isAuthenticated()?res.render('post',{post,isLogin:true,writername:user.name,writeremail:user.email,user:req.user}):res.render('post',{post,writername:user.name,writeremail:user.email,isLogin:false});
                    }
                })
            }
    })
